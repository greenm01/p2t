...
pub fn build(b: *std.Build) void {
    ...
    const consumer_smoke_mod = b.createModule(.{
        .root_source_file = b.path("tests/package_consumer.zig"),
        .target = target,
        .optimize = optimize,
    });
    consumer_smoke_mod.addImport("p2t", lib_mod);
    const consumer_smoke_tests = b.addTest(.{ .root_module = consumer_smoke_mod });
    const run_consumer_smoke_tests = b.addRunArtifact(consumer_smoke_tests);
    test_step.dependOn(&run_consumer_smoke_tests.step);
